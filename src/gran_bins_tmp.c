...
namespace Gran_Bins_Tmp
{
  int num_bins = -1;

  int gran_bins_tmp_init()
  {
  ...
  int gran_bins_tmp_clean()
  {
    return -1; 
  }

  int gran_bins_tmp_thread_req()
  {
    return 1;
  }

  int gran_bins_tmp_stencil_init(gran_request** gran_req, void*** recv_bufs, void** send_bufs, int* recv_ids, int* send_ids, int* num_entries, int* entry_size, int stencil)
  {
    ...
    gran_bins_tmp_req* req = (gran_bins_tmp_req*) malloc(sizeof(gran_bins_tmp_req));
  ...
  int gran_bins_tmp_stencil_start(gran_request* gran_req)
  {
    gran_bins_tmp_req* req = (gran_bins_tmp_req*) gran_req;
  ...
  int gran_bins_tmp_stencil_ready(gran_request* gran_req, int send_dir, int entry)
  {
    int err = 0;
    gran_bins_tmp_req* req = (gran_bins_tmp_req*) gran_req;
    ...
    if(capture == req->bin_thresh[send_dir]) 
    {
      ...
      if(bc_capture == num_bins - 1)
      {
        ...
        if (-1 == last_bin)
        {
          ...
          if(err)
          {
            int id;
            comm_get_id(&id);
            printf("(%d) Error: complete buffer send in bins_tmp: comm_channel_send_range(%p,%d,%d)\n", id, req->comm_req[send_dir], 0, num_bins - 1);
            return -1;
          }
        }
        else
        {
          ...
          if(err)
          {
            int id;
            comm_get_id(&id);
            printf("(%d) Error: first send in bins_tmp: comm_channel_send_range(%p,%d,%d)\n", id, req->comm_req[send_dir], 0, last_bin - 1);
            return -1;
          }
          if(last_bin != num_bins - 1)
          {
            ...
            if(err)
            {
              int id;
              comm_get_id(&id);
              printf("(%d) Error: second send in bins_tmp: comm_channel_send_range(%p,%d,%d)\n", id, req->comm_req[send_dir], last_bin + 1, num_bins - 1);
              return -1;
            }
          }
        }
      ...
      else if(bc_capture == num_bins)
      {
        ...
        if(err)
        {
          int id;
          comm_get_id(&id);
          printf("(%d) Error: final send in bins_tmp: comm_channel_send(%p,%d)\n", id, req->comm_req[send_dir], bin);
          return -1;
        }
      }
    }
  ...
  int gran_bins_tmp_stencil_end(gran_request* gran_req)
  {
    gran_bins_tmp_req* req = (gran_bins_tmp_req*) gran_req;
    int i, err;
  ...
  int gran_bins_tmp_stencil_finish(gran_request* gran_req)
  {
    gran_bins_tmp_req* req = (gran_bins_tmp_req*) gran_req;
    int i, err;
    for (i = 0; i < req->stencil; i++)
  ...
  int gran_bins_tmp_load(gran_funcs* funcs)
  {
    funcs->init = &gran_bins_tmp_init;
    funcs->clean = &gran_bins_tmp_clean;
    funcs->thread_req = &gran_bins_tmp_thread_req;
    funcs->stencil_init = &gran_bins_tmp_stencil_init;
    funcs->stencil_start = &gran_bins_tmp_stencil_start;
    funcs->stencil_ready = &gran_bins_tmp_stencil_ready;
    funcs->stencil_end = &gran_bins_tmp_stencil_end;
    funcs->stencil_finish = &gran_bins_tmp_stencil_finish;
    return 0;
  }
} 
