 ...
 */
void kernel_sample_print_matrix(kernel_sample_data* kernel_data)
{
  int i;
...
void kernel_sample_write(kernel_sample_data* kernel_data, int iter)                                                             
{
 ...
 */
int kernel_sample_process_map(int num_procs, int* x, int* y)
{
  int i;
  int best = 1;
...
int kernel_sample_init(void** kernel_data, int id, int job_size)
{
  int i, j;
  kernel_sample_data* new_data = (kernel_sample_data*) malloc(sizeof(kernel_sample_data));
  ...
  kernel_sample_process_map(job_size, &new_data->proc_x, &new_data->proc_y); 
  if(id == 0)
  {
  ...
  if(DEBUG) kernel_sample_print_matrix(new_data);
  if(WRITE) kernel_sample_write(new_data, -1);
...
int kernel_sample_run(void* _kernel_data)
{
  kernel_sample_data* kernel_data = (kernel_sample_data*)_kernel_data;
  comm_barrier();
  if(kernel_data->id == 0) printf("starting computation\n");
  #if TIMING == 1
    std::clock_t t_start;
    if(kernel_data->id == 0) t_start = std::clock();
  #endif

  int t, i, j;
  int north_ready, east_ready, south_ready, west_ready;
  for (t = 0; t < kernel_data->iters; t++)
  {
    ...
    {
      for (j = 0; j < kernel_data->halo_y; j++)
      {
    ...
    if(DEBUG) kernel_sample_print_matrix(kernel_data);
    if(WRITE) kernel_sample_write(kernel_data, t);
...
int kernel_sample_clean(void* _kernel_data)
{
  kernel_sample_data* kernel_data = (kernel_sample_data*)_kernel_data;
  int i;
  for(i = 0; i < kernel_data->halo_x; i++)
...
int kernel_sample_load(kernelfuncs* funcs)
{
  funcs->init  = &kernel_sample_init;
  funcs->run   = &kernel_sample_run;
  funcs->clean = &kernel_sample_clean;
  return 0;
}
